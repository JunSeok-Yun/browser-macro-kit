...
src/
index.ts        - 진입점. 재시도 루프 조율
browser.ts      - 영구 브라우저 컨텍스트 팩토리 (신규)
...
1. **CreepJS 검증** — 현재 지문/IP 신뢰 점수 종합 확인
2. **쿠팡 검색 → 상품 진입 시퀀스 구현** — 검색창 입력 후 상품 클릭까지 행동 모방 (2단계 시작)
3. **베지어 곡선 마우스 이동** — 자연스러운 마우스 궤적 구현
4. **랜덤 스크롤 체류 로직** — 페이지 진입 후 인간형 스크롤 패턴
